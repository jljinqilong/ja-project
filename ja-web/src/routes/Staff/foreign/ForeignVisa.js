...
import AddForm from '../../../components/Staff/ForeignVisa/ModalAddForm';
import EditForm from '../../../components/Staff/ForeignVisa/ModalEditForm';
import { del, pageList } from '../../../services/foreignVisa';

import styles from './ForeignVisa.less';
import { serverUrlPre } from '../../../utils/request';
import {getToken, hasAccessKey} from '../../../utils/authority';

export default class ForeignVisa extends PureComponent {
  state = {
    data: {},
   ...
   * @param rowId
   */
  showDelForeignVisaConfirm = rowId => {
    const rt = this.refreshTable;
    Modal.confirm({
      title: '删除确认',
      content: '确定删除此外籍员工工作信息吗？',
      okText: '确定',
      okType: 'danger',
      cancelText: '取消',
      onOk() {
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: '年检时间',
        dataIndex: 'annualInspectionDate',
      },
      {
        title: '附件',
        render: (text, record) => {
      ...
      },
      {
        ...
        render: (text, record) => (
          <Fragment>
            ...
            {hasAccessKey(`staff.baseInfo.update`) && (
            <a title="删除" onClick={() => this.showDelForeignVisaConfirm(record.rowId)}>
              <Icon type="delete" />
            </a>
            )}
          </Fragment>
      ...
      <div>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.title}>工作许可证信息</div>
