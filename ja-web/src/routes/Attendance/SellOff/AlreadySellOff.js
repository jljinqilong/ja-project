...
import SearchForm from '../../../components/Attendance/SellOff/SearchForm2';

import { delPersonalLeave, pageList, getById } from '../../../services/personalLeave';
import styles from './AlreadySellOff.less';
...
export default class AlreadySellOff extends PureComponent {
  state = {
    data: [],
    staffAllList: [],
    holidayTypeList:[],
  ...
  refreshTable = params => {
    ...
    params.status = '1';
    this.props.refreshAlreadySellOffTable(params);
    // pageList(params)
    //   .then(response => {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        title: '实际开始日期',
        render: (text, record) => {
          return moment(record.realStartDate).format('YYYY-MM-DD HH:mm');
        },
      },
      {
        title: '实际结束日期',
        render: (text, record) => {
          return moment(record.realEndDate).format('YYYY-MM-DD HH:mm');
        },
      },
      {
        title: '实际时数',
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <StandardTable
              ...
              loading={loading}
              data={this.props.alreadySellOffData}
              columns={columns}
