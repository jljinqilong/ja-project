...
export default {
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put }) {
      const response = yield call(listInquiriesAppraisal, payload);
      yield put({
        type: 'save',
        payload: {
    ...
    *turnContract({ rowId, successCallback, failCallback }, { call }) {
    ...
    /* 询单评审确认（通过、驳回） */
    *fetchAffrimAppraisal({ params, successCallback, failCallback }, { call }) {
      const response = yield call(affrimAppraisal, params);
      if (response && response.code === 200) {
        if (successCallback) successCallback();
      } else if (failCallback) failCallback();
    },
    ...
    // =========== 查询询单历史记录 ============
    *fetchInquiriesHistory({ rowId }, { call, put }) {
      const response = yield call(listInquiriesRecordDetail, rowId);
      yield put({
