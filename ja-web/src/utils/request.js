...
export const serverUrlPre = 'http://121.36.102.155:8080';
...
function checkStatus(response) {
  ...
  // TODO 调整 400 ~ 500 错误码，区别 业务异常code 值
...
export default function request(url, options) {
  ...
  if (
    ...
    // defaultOptions.body = { token: getToken() };
    ...
    if (/^.*\?.+=.+$/.test(url)) {
      url = `${url}&lang=${getLang()}`; // eslint-disable-line no-param-reassign
