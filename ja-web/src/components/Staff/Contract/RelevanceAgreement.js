...
import { getByAgreementNo } from '../../../services/agreement';
...
class RelevanceAgreement extends PureComponent {
  state = {
    staffId: '',
  };
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.currentAgreementNo != nextProps.currentAgreementNo &&
      nextProps.currentAgreementNo !== -1
    ) {
      getByAgreementNo(nextProps.currentAgreementNo)
        .then(data => {
  ...
  render() {
    ...
    return (
      <Modal
        title="协议详情"
        visible={this.props.agreementDetails}
        width={1200}
        footer={null}
...
export default Form.create({})(RelevanceAgreement);
