...
class ModalDiscontinueDateForm extends PureComponent {
  state = {
    staffId: '',
    signDate: '',
    contractDateStart: '',
    contractDateEnd: '',
    discontinueDate: moment(new Date()).format('YYYY-MM-DD'),
    filePath: '',
    fileList: [],
  };
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.currentDiscontinueUserId != nextProps.currentDiscontinueUserId &&
      nextProps.currentDiscontinueUserId > 0
    ) {
      getById(nextProps.currentDiscontinueUserId)
        .then(data => {
  ...
  handleSure = () => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      fieldsValue.rowId = this.props.currentDiscontinueUserId;
      ...
      fieldsValue.discontinueDate = this.state.discontinueDate;
      ...
      updateContract(fieldsValue)
        .then(data => {
          if (data.code === 200) {
            ...
            this.props.handleModalVisible(false, 6, -1);
  ...
  render() {
    ...
    return (
      <Modal
        title="中止合同"
        visible={this.props.modalDiscontinue}
        onOk={this.handleSure}
        okText={`保存`}
        width={1200}
        onCancel={() => this.props.handleModalVisible(false, 6, -1)}
      >
        <Form>
          ...
          <Row gutter={24}>
            <Col span={12}>
              <Form.Item labelCol={{ span: 5 }} wrapperCol={{ span: 16 }} label="中止日期">
                {getFieldDecorator('discontinueDate', {
                  initialValue: moment(this.state.discontinueDate),
                  rules: [{ required: true, message: '请选择中止日期' }],
                })(
                  <DatePicker
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="中止原因">
                {getFieldDecorator('discontinueReason', {
                  rules: [{ required: true, message: '请输入中止原因' }],
                })(<Input placeholder="请输入中止原因" maxLength="100" />)}
              </Form.Item>
            </Col>
            <Col span={12}>
...
export default Form.create({})(ModalDiscontinueDateForm);
