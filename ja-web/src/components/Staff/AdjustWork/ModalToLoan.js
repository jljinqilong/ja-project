...
class ModalToLoan extends PureComponent {
  state = {
    staffId: '',
  ...
  handleDatePickerChangeStartDate = (date, dateString) => {
    this.setState({
      temporarilyStartDate: dateString,
    });
  };
    ...
    getAdjustmentBy(staffId1,changeType).then(data => {
      if(!!data && data.data.length >0) {
        if (!!data.data[0].realEndTime) {
          if (moment(data.data[0].realEndTime).format('YYYY-MM-DD') >= today) {
            message.warn('该员工目前正在借调中，不能再次借调！');
            return;
          }
        } else {
      ...
      getAdjustmentBy(staffId1,changeType1).then(data => {
        if(!!data && data.data.length >0) {
          if (!!data.data[0].realEndTime) {
            if (moment(data.data[0].realEndTime).format('YYYY-MM-DD') >= today) {
              message.warn('该员工目前正在外派中，不能再次借调！');
              return;
            }
          } else {
        ...
        if(this.props.jobStatus === '在职' || this.props.jobStatus === '待岗') {
          form.validateFields((err, fieldsValue) => {
            ...
            if (this.state.temporarilyStartDate > this.state.temporarilyEndDate) {
            ...
            fieldsValue.staffId = this.props.staffId,
              ...
              fieldsValue.temporarilyStartDate = this.state.temporarilyStartDate;
            fieldsValue.temporarilyEndDate = this.state.temporarilyEndDate;
            fieldsValue.currentId = this.state.currentId;
            fieldsValue.changeType = 'TEMPORARILY',
              Modal.confirm({
                title: '添加确认',
                content: '确定添加此条记录吗？',
                okText: '确定',
  ...
  getStaffNo = () => {
    ...
    form.resetFields();
    form.setFieldsValue({
      temporarilyType: values.temporarilyType,
      ...
      temporarilyStartDate:values.temporarilyStartDate,
      temporarilyEndDate:values.temporarilyEndDate,
  ...
  render() {
    ...
    return (
      <Fragment>
        <Card title={'借调信息'} bordered={false} extra={action}>
          <Row gutter={24}>
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调类型">
                {getFieldDecorator('temporarilyType', {
                  rules: [{ required: true, message: '请选择借调类型' }],
                })(
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调基地">
                {getFieldDecorator('newBase', {
                  rules: [{ required: true, message: '请选择借调基地' }],
                })(
                  <Select style={{ width: 300 }}>
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调部门">
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调职衔">
                {getFieldDecorator('newPosition', {
                  rules: [{ required: true, message: '请选择借调职衔' }],
                })(
                  <Select
                    placeholder={`可模糊搜索`}
                    showSearch
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调职等/赋值名称">
                {getFieldDecorator('newGrade', {
                  rules: [{ required: true, message: '请选择借调职等/赋值名称' }],
                })(
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="借调职级">
                {getFieldDecorator('newRank', {
                  rules: [{ required: true, message: '请选择借调职级' }],
                })(
                  <Select style={{ width: 300 }}>
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 6 }} wrapperCol={{ span: 17 }} label="开始时间">
                {getFieldDecorator('temporarilyStartDate', {
...
export default Form.create({})(ModalToLoan);
