...
class ModalReturnRehire extends PureComponent {
  state = {
    staffId: '',
    workDate: '',
    baseId: '',
  ...
  componentDidMount() {
      ...
      let changeType = 'RETIRE'
      getAdjustmentWorkList(staffId,changeType).then(data => {
        if(!!data) {
          this.setState({
            retireDate: moment(data.data.retireDate).format('YYYY-MM-DD'),
          });
        }
      });
  ...
  handleSure = () => {
    ...
    if(this.props.jobStatus === '退休') {
      form.validateFields((err, fieldsValue) => {
        if (err) return;
        if (this.state.workDate < this.state.retireDate) {
          message.warn('用工日期不能小于退休日期！退休日期为:'+this.state.retireDate);
          return;
        }
        ...
        fieldsValue.changeType = 'RETURN_REHIRE',
          Modal.confirm({
            title: '添加确认',
            content: '确定添加此条记录吗？',
            okText: '确定',
  ...
  render() {
    ...
    return (
      <Fragment>
        <Card title={'返聘信息'} bordered={false} extra={action}>
          <Row gutter={24}>
            <Col span={12}>
          ...
          <Row gutter={24}>
            ...
            <Col span={12}>
              <Form.Item labelCol={{ span: 7 }} wrapperCol={{ span: 17 }} label="入职前工龄">
                ...
                })(<Input style={{ width: 300 }} maxLength={2} onKeyUp={this.handleChangeAge.bind(this)}/>)}
              </Form.Item>
            </Col>
          </Row>
        </Card>
...
export default Form.create({})(ModalReturnRehire);
