...
import { Form, Input, Select, Row, Col, message, Card, Popconfirm, Icon, Alert } from 'antd';
...
@Form.create()
export default class EditInquiriesBaseForm extends PureComponent {
  constructor(props) {
    super(props);
  ...
  get = rowId => {
    if (rowId) {
      getInquiriesById(rowId)
        .then(resp => {
          const { data } = resp;
          const { inquiriesProductList, inquiriesDeliveryTimeList } = data;
          let newInquiriesDeliveryTimeDetails;
          let newInquiriesProductDetails;

          // 回显询单交期
          if (inquiriesDeliveryTimeList && Array.isArray(inquiriesDeliveryTimeList)) {
            newInquiriesDeliveryTimeDetails = inquiriesDeliveryTimeList.map(item => {
              return {
                ...item,
          ...
          // 回显询单产品
          if (inquiriesProductList && Array.isArray(inquiriesProductList)) {
            newInquiriesProductDetails = inquiriesProductList.map(item => {
              return {
                ...item,
                rowKey: this.getNextSeq(),
              };
          ...
          return {
            ...data,
            inquiriesDeliveryTimeList: newInquiriesDeliveryTimeDetails,
            inquiriesProductList: newInquiriesProductDetails,
          };
        })
        .then(data => {
          // 默认的国省市\ 1, 7 ,247
          ...
          // 更新产品列表，用于计算总金额
          this.updateProducts(data.inquiriesProductList);
          this.updateDelivery(data.inquiriesDeliveryTimeList);
        })
        .catch(() => {
  ...
  handleOK = () => {
    ...
    let { inquiriesDeliveryTimeList } = this.state;
    const { detail, totalAmount, totalPower, inquiriesProductList, countryProvCity } = this.state;
    const { estimatedSigningDate, inquiryTime } = detail;

    // 时间转换为 string
    if (inquiriesDeliveryTimeList) {
      const newDeliverty = inquiriesDeliveryTimeList.map(item => ({
        ...item,
        deliveryTime: moment(item.deliveryTime).format('YYYY-MM-DD'),
      }));
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const inquiries = {
        ...
        inquiryTime,
        ...fieldsValue,
        estimatedSigningDate, // 时间转换 string
        totalAmount,
        totalPower,
        inquiriesProductList,
        inquiriesDeliveryTimeList,
  ...
  updateDelivery = delivery => {
    if (delivery) {
      const newDeliverty = delivery.map(item => ({
        ...item,
        createdTime: moment(item.createdTime, 'YYYY-MM-DD HH:mm:ss'),
          ...
          <Col span={12}>
            <FormItem label="区域" labelCol={labelCol} wrapperCol={wrapperCol}>
              {getFieldDecorator('saleAreaId', {
                ...
                initialValue: detail.saleAreaId,
              })(
                <Select placeholder="请选择">
                  {areaList.map(d => (
                    <Select.Option key={d.rowId} value={d.rowId}>
          ...
          <Col span={12}>
            <FormItem label="项目/分销" labelCol={labelCol} wrapperCol={wrapperCol}>
              {getFieldDecorator('projectDistributionId', {
                ...
                initialValue: detail.projectDistributionId,
              })(
                <Select placeholder="请选择">
          ...
          <Col span={12}>
            <FormItem label="发货地" labelCol={labelCol} wrapperCol={wrapperCol}>
              {getFieldDecorator('dispatchPlaceId', {
                ...
                initialValue: detail.dispatchPlaceId,
              })(
                <Select placeholder="请选择">
          ...
          <Col span={12}>
            <FormItem label="币种" labelCol={labelCol} wrapperCol={wrapperCol}>
              {getFieldDecorator('currencyId', {
                ...
                initialValue: detail.currencyId,
              })(
                <Select placeholder="请选择">
                  {currencyList.map(d => (
                    <Select.Option key={d.rowId} value={d.rowId}>
  ...
  inquiriesDeliveryInfo() {
    const { inquiriesDeliveryTimeList, deliveryTime } = this.state;
    const { form } = this.props;
          ...
          inputtype:
            col.dataIndex === 'deliveryTime'
              ...
              : col.dataIndex === 'num'
                ? 'number'
                : 'input',
          datasource: col.dataIndex === 'deliveryTime' ? moment(deliveryTime, 'YYYY-MM-DD') : null,
          form,
          rules: col.rules,
        }),
      };
  ...
  render() {
    const { modalVisible, totalAmount, totalPower } = this.props;
